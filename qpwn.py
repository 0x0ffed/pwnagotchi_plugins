...
class Qpwn(plugins.Plugin):
    __author__ = '0x0ffed'
    __version__ = '1.1.4'
    __license__ = 'GPL3'
    __description__ = 'Learns. Scores. Pwns. Q-driven.'

    def __init__(self):
        self.memory_path = '/etc/pwnagotchi/qpwn.json'
        self.qtable_path = '/etc/pwnagotchi/qpwn_brain.json'
        self.aps = {}
        self.qtable = {}
    ...
    def _load_memory(self):
        try:
            if os.path.exists(self.memory_path):
                ...
                logging.info(f"[qpwn] Loaded memory with {len(self.aps)} APs")
            
            if os.path.exists(self.qtable_path):
                with open(self.qtable_path, 'r') as f:
                    self.qtable = json.load(f)
                logging.info("[qpwn] Q-table loaded")
            ...
            logging.error(f"[qpwn] Failed to load memory: {e}")
            self.aps = {}
            self.qtable = {}
    ...
    def _save_memory(self):
        try:            
            ...
            logging.error(f"[qpwn] Failed to save memory: {e}")
        ...
        with self.lock:
            ...
            if client_mac not in clients:
                ...
                if vendor != "unknown":
                    ...
                    clients[client_mac]["vendor"] = "unknown"
    ...
    def _execute_attack(self, agent, ap, client):
        try:
            ...
            if self._is_whitelisted(ap_mac, ssid):
                logging.debug(f"[qpwn] Skipping whitelisted target: {ssid} ({ap_mac})")
                return
            ...
            with self.lock:
                ...
                if not ap_data:
                    logging.warning(f"[qpwn] AP {ap_mac} not tracked; skipping attack")
                    return
                client_data = ap_data["clients"].get(client_mac)
                if not client_data:
                    logging.warning(f"[qpwn] Client {client_mac} not tracked under {ap_mac}; skipping")
                    return
                ...
                if client_data["score"] < dynamic_threshold:
                    logging.debug(f"[qpwn] Skipping low-score target {client_mac} ({client_data['score']:.2f} < dyn {dynamic_threshold:.2f})")
                    return
            ...
            logging.info(f"[qpwn] Attacking {client_mac} ({vendor}) on {ssid} ({ap_mac}) score={client_data['score']:.2f}")
            ...
            logging.error(f"[qpwn] Attack failed: {e}")
    ...
    def _delayed_retry_attack(self, agent, ap, client, delay):
        try:
            ...
            logging.error(f"[qpwn] Retry attack failed: {e}")
    ...
    def _q_learn(self, blind, handshakes):
        try:
            ...
            logging.error(f"[qpwn] Q-learning failed: {e}")
    ...
    def _select_optimal_channel(self):
        try:
            ...
            logging.error(f"[qpwn] Channel selection failed: {e}")
            return random.choice(self.default_channels)
            ...
            for ap_mac, ap in self.aps.items():
                if (
                    ...
                    if random.random() < self.broadcast_deauth_prob:
                        ...
                        logging.info(f"[qpwn] Broadcast deauth sent to {ap_mac}")
                    else:
                        logging.debug(f"[qpwn] Skipped broadcast deauth to {ap_mac} (rate limited)")
        except Exception as e:
            logging.error(f"[qpwn] Broadcast deauth failed: {e}")
    ...
    def _render_main_page(self):
        try:
            ...
            logging.error(f"[qpwn] {error_msg}")
    ...
    def on_webhook(self, path, request):
        try:
            ...
            logging.error(f"[qpwn] Webhook error: {error_msg}")
    ...
    def on_loaded(self):
        try:
            ...
            logging.info("[qpwn] Loaded with web UI at /plugins/qpwn/")
        except Exception as e:
            logging.error(f"[qpwn] Error during loading: {str(e)}")

    def on_unload(self, ui):
        self._save_memory()
        self.executor.shutdown(wait=False)
        if self.ui_elements_added:
            ui.remove_element('qpwn_stats')
        logging.info("[qpwn] Unloaded")

    def on_ui_setup(self, ui):
        ui.add_element('qpwn_stats', Text(position=(195, 109), value="", color=0, font=fonts.Bold))
    ...
    def on_ui_update(self, ui):
        ...
        recon_label = self.RECON_LABELS.get(recon_key, "QPWN")
        ui.set('qpwn_stats', recon_label)
    ...
    def on_bcap_wifi_ap_new(self, agent, event):
        ...
        logging.info(f"[qpwn] New AP: {ssid} ({ap_mac}) on channel {ap.get('channel')}")
    ...
    def on_handshake(self, agent, filename, ap, client):
        ...
        if ap_mac in self.aps and client_mac in self.aps[ap_mac]["clients"]:
            ...
            logging.info(f"[qpwn] Handshake captured from {client_mac} on {ap_mac}")
            self.executor.submit(self._execute_attack, agent, ap, client)
    ...
    def on_epoch(self, agent, epoch, epoch_data):
        ...
        logging.info(f"[qpwn] Switched to optimal channel {best_channel}")
